...
/**
 * Dark slideshow hero for the שיתופי פעולה page — mirrors LiabahHero's
 * structure (Ken Burns slideshow, intro timeline, scroll cue) with the
 * "קמפוס חמש אצבעות / בית תוכן והכשרות" messaging.
 */
export default function CollabsHero({ onRegister }) {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.fromTo('.ch-eyebrow', { y: 16, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5, ease: 'power2.out' }, 0.1)
        .fromTo('.ch-title', { y: 40, opacity: 0, filter: 'blur(10px)' }, { y: 0, opacity: 1, filter: 'blur(0px)', duration: 0.8, ease: 'power3.out' }, '-=0.2')
        .fromTo('.ch-accent', { scaleX: 0, opacity: 0 }, { scaleX: 1, opacity: 1, duration: 0.35, ease: 'power2.inOut' }, '-=0.2')
        .fromTo('.ch-sub', { y: 16, opacity: 0 }, { y: 0, opacity: 1, duration: 0.4, ease: 'power2.out' }, '-=0.05')
        .fromTo('.ch-cta', { y: 14, opacity: 0 }, { y: 0, opacity: 1, stagger: 0.08, duration: 0.4, ease: 'power2.out' }, '-=0.05')
        .fromTo('.ch-chrome', { opacity: 0 }, { opacity: 1, duration: 0.6, ease: 'power2.out' }, '-=0.1')
    }, ref)
    return () => ctx.revert()
  }, [])
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex-1 flex items-center">
        <div className="w-full max-w-screen-xl mx-auto px-6 md:px-12 lg:px-20 pt-28 md:pt-32 pb-36">
          <div className="text-center">
            <p className="ch-eyebrow font-heebo text-white/55 tracking-[0.3em] text-sm md:text-base mb-4">
              בית תוכן והכשרות
            </p>
            <h1
              className="ch-title leading-[0.92] tracking-tight"
              style={{ fontFamily: "'RagMarom', sans-serif", fontSize: 'clamp(3rem, 9vw, 9rem)' }}
            >
              קמפוס חמש אצבעות
            </h1>
            <div
              className="ch-accent mx-auto mt-6 rounded-full bg-orange"
              style={{ height: '3px', width: 'clamp(7rem, 18vw, 18rem)', transformOrigin: 'center' }}
            />
            <p
              className="ch-sub font-heebo text-white/85 mt-7 mx-auto max-w-3xl leading-snug"
      ...
      <a
        ...
        className="ch-chrome scroll-indicator absolute left-1/2 bottom-4 z-10 text-white/60 hover:text-white transition-colors"
      >
        <ChevronDown size={26} />
      </a>
    </section>

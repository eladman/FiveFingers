 ...
 * A slow photo push-in behind a heavy scrim, a magnetic primary CTA,
 * and the partnership contact as one quiet line instead of three cards.
 */
export default function CollabsFinaleV2({ onRegister }) {
    ...
    >
      <div className="absolute inset-0 overflow-hidden">
        <img
          ...
          loading="lazy"
          decoding="async"
          className="cfin-photo absolute inset-0 w-full h-full object-cover will-change-transform"
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto px-6 sm:px-10 text-center py-28">
        <h2
          className="cfin-el font-ragmarom text-white leading-[1.0]"
          style={{ fontSize: 'clamp(2.6rem, 6vw, 6rem)', textShadow: '0 4px 32px rgba(0,0,0,0.9)', textWrap: 'balance' }}
        >
          רוצים להביא את חמש אצבעות <span className="text-orange">לארגון שלכם?</span>
        </h2>
        <p
          className="cfin-el font-heebo text-white/80 mt-6 max-w-xl mx-auto leading-relaxed"
        ...
        <div className="cfin-el mt-11 flex flex-wrap items-center justify-center gap-5">
        ...
        <div
          className="cfin-el mt-14 pt-6 border-t border-white/15 flex flex-wrap items-center justify-center gap-x-8 gap-y-2 font-heebo text-sm md:text-base"
          style={{ textShadow: '0 1px 10px rgba(0,0,0,0.7)' }}
        >
          <a href={`tel:${PHONE.replace(/-/g, '')}`} dir="ltr" className="tap-safe text-white/60 hover:text-white transition-colors">
            {PHONE}
          </a>
          <span className="w-1 h-1 rounded-full bg-orange inline-block" aria-hidden="true" />
          <a href={`mailto:${EMAIL}`} dir="ltr" className="tap-safe text-white/60 hover:text-white transition-colors">
            {EMAIL}
          </a>
        </div>
      </div>
